...
# Accent color-linked keys — status colors (ACC, GREEN, RED…) remain fixed
_HUE_LINKED = (
...
def apply_ui_accent(accent_hex: str) -> bool:
    """
    Re-derives the entire turquoise-family palette based on the selected
accent color (hue shifting — brightness/saturation ratios are preserved
so the design remains consistent).

Painted elements (HUD, waveform, metrics) receive the new color on the
next frame; stylesheet-based panels receive it when they are rebuilt.
"""
    import colorsys
    ...
    dh   = acc_h - base_h
    grey = acc_s < 0.08   # Near-gray accent → the entire theme is desaturated
...
def current_palette() -> dict[str, str]:
    """A snapshot of the colors in the class that are linked to the accent."""
    return {k: getattr(C, k) for k in _HUE_LINKED}


def retheme_all_widgets(old: dict[str, str], new: dict[str, str]) -> None:
    """
    Live full theme change. Replaces the old palette colors with the new ones
    in every widget's stylesheet and repaints them. This ensures the color
    change is applied instantly across the entire interface, including
    panels, buttons, and borders, without restarting the application.
    """
...
def _nvml_gpu_windows() -> float:
    """Return NVIDIA GPU utilisation % using nvml.dll directly _ zero subprocess."""
...
class _SysMetrics:
    ...
    def _get_gpu(self) -> float:
        # pynvml _ subprocess-free, works on all platforms if installed
        try:
            import pynvml  # type: ignore
            pynvml.nvmlInit()
            h = pynvml.nvmlDeviceGetHandleByIndex(0)
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "O.N.X.Y", parent=None):
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "ONXY"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("ONXY:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for ONXY", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure O.N.X.Y. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class HueWheel(QWidget):
    """
    Circular color selector. The user can drag the small white handle
    around the ring to select any hue.
    ...
    def _hue_from_pos(self, pos: QPointF) -> float:
        ...
        dx = pos.x() - c.x()
        dy = c.y() - pos.y()          # Convert screen y-axis (downward) to mathematical y-axis
        ang = math.atan2(dy, dx)      # [-π, π], counterclockwise
...
class CustomizeOverlay(QWidget):
    """Floating overlay for changing assistant name, user name and UI colour."""

    saved = pyqtSignal(str, str, str)   # assistant_name, user_name, ui_color
    _OW, _OH = 400, 500

    def __init__(self, assistant_name="ONXY", user_name="",
        ...
        # ── UI colour _ Color wheel ─────────────────────────────────────────────────────
        lay.addSpacing(4)
        clr_hdr = QHBoxLayout()
        clr_hdr.addWidget(_lbl("UI COLOUR  _  drag the handle", 8,
                               color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        clr_hdr.addStretch()
    ...
    def _on_wheel_pick(self, hx: str):
        # During dragging: update the hex field, but do not apply the theme yet
        self._sel_color = hx
        self._hex_input.blockSignals(True)
    ...
    def _on_wheel_commit(self, hx: str):
        # When the handle is released: live-preview the entire UI
    ...
    def _cancel(self):
        # If a preview was applied, revert to the initial color
        if self.on_preview and self._sel_color != self._initial_color:
            self.on_preview(self._initial_color)
        self.hide()

    def _save(self):
        name = self._name_input.text().strip() or "ONXY"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied _ offers quick ONXY actions."""
...
class RemoteKeyOverlay(QWidget):
    """Floating overlay for QR code for instant phone pairing + manual key fallback."""
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected _ ONXY ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "ONXY").strip()
        _display = self._assistant_name.upper()

        # Apply the saved UI color BEFORE panels/stylesheets are initialized
        _ui_color = (_cfg.get("ui_color") or "").strip()
        if _ui_color and _ui_color.lower() != DEFAULT_UI_COLOR:
            apply_ui_accent(_ui_color)

        self.setWindowTitle(f"{_display} _ ONXY")
        ...
        self.on_interrupt      = None   # callable: () -> None _ stop ONXY mid-speech
        self._muted            = False
        self._current_file: str | None = None
        ...
        # Live camera container _ replaces HUD when camera stream is active
        _cam_cont = QWidget()
    ...
    # ------------------------------------------------------------------
    # Icon generation _ arc-reactor style, rendered with Pillow
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        wscript.exe is a GUI-mode host _ it never opens a console window.
        """
        ...
        try:
            ...
            sc.Description      = "O.N.X.Y AI Assistant"
        ...
        #    GUI-mode executable _ never opens a console window) ───────────────────────
        vbs = "\n".join([
    ...
    def _get_desktop_dir() -> Path:
        ...
        if _os == "Windows":
            # ── 1) SHGetKnownFolderPath(FOLDERID_Desktop) _ the canonical
    ...
    def _create_desktop_shortcut(self):
        ...
        # Arc-reactor icon (.ico _ also exported as .png for Linux/macOS)
        ico_path = Path(__file__).resolve().parent / "config" / "ONXY.ico"
        if not ico_path.exists():
            self._build_ONXY_icon(ico_path)
        ...
        try:
            ...
            if _os == "Windows":
                ...
                lnk      = str(desktop / "O.N.X.Y.lnk")
                icon_loc = str(ico_path) if ico_path.exists() else f"{target},0"
                self._create_lnk_windows(lnk, target, str(script),
                                         str(script.parent), icon_loc)

            # ── macOS _ proper .app bundle (no Terminal window) ──────────────────────
            elif _os == "Darwin":
                app     = desktop / "O.N.X.Y.app"
                mac_dir = app / "Contents" / "MacOS"
                res_dir = app / "Contents" / "Resources"
                mac_dir.mkdir(parents=True, exist_ok=True)
                res_dir.mkdir(exist_ok=True)

                # Launcher executable (bash _ runs as background process,
                # macOS does NOT open Terminal for executables inside .app bundles)
                launcher = mac_dir / "ONXY"
                launcher.write_text(
                    "#!/usr/bin/env bash\n"
                    f'cd "{script.parent}"\n'
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>ONXY</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.ONXY.assistant</string>\n'
                    '  <key>CFBundleName</key><string>O.N.X.Y</string>\n'
            ...
            self._log.append_log(f"ERR: Shortcut failed _ {e}")
    ...
    def resizeEvent(self, event):
        ...
        # Camera preview _ bottom-right corner of the center/HUD area
        pw = _CameraPreview._W
        ...
        # Clipboard panel _ bottom-center
        if hasattr(self, '_clipboard_panel') and self._clipboard_panel.isVisible():
            self._position_clipboard_panel()
        # Quick drawer _ reposition if open
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("ONXY", C.PRI_DIM))
        lay.addSpacing(8)
        ...
        _sub_text = ("Built to Think. Engineered to Execute."
                     if _disp in ("ONXY", "O.N.X.Y")
                     else "Built to Think. Engineered to Execute.")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._file_hint = QLabel("No file loaded _ drop or click above to upload")
    ...
    def _build_content_panel(self) -> QWidget:
        """
        Collapsible panel below the HUD _ shows search results, news, briefings.
    ...
    def _show_content(self, title: str, text: str):
        """Slot _ runs on Qt main thread. Updates and shows the content panel."""
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By RDV", C.PRI_DIM))
        return w
    ...
    def _open_remote(self):
        if not self.on_remote_clicked:
            self._log.append_log("SYS: Dashboard not running _ remote unavailable.")
            return
   ...
   # ── Auto-start ────────────────────────────────────────────────────────────
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "ONXY_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.ONXY.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "ONXY.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "ONXY_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "ONXY_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.ONXY.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.ONXY.assistant</string>\n'
            ...
            self._log.append_log(f"ERR: Auto-start failed _ {e}")
    ...
    def _preview_ui_color(self, hex_color: str):
        """Live preview — recolors the entire UI without writing to config."""
        old = current_palette()
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "ONXY"
        ...
        if ui_color:
            ...
            if apply_ui_accent(ui_color):
                # Live-retheme the entire interface (panels, buttons, borders, HUD)
                retheme_all_widgets(old, current_palette())
        ...
        try:
            ...
            self._log.append_log(f"SYS: Identity updated _ {display}")
            if color_changed:
                self._log.append_log(f"SYS: UI colour applied _ {ui_color}")
        except Exception as e:
            self._log.append_log(f"ERR: Config save failed _ {e}")
