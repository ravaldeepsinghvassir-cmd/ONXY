...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".ONXY_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".ONXY_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ONXY profile")
                ONXY = str(Path.home() / ".ONXY_profiles" / "firefox_ONXY")
                Path(ONXY).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(ONXY, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".ONXY_profiles" / "safari")
        ...
        # ONXY otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        ONXY_profile = str(Path.home() / ".ONXY_profiles" / self.browser_name)
        Path(ONXY_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with ONXY profile: {ONXY_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(ONXY_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with ONXY profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "ONXY_screenshot.png")
