"""
dashboard/server.py — LUCKY Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "LUCKY Uploads",
        Path.home() / "Documents" / "LUCKY Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'LUCKY-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"LUCKY Dashboard Port {port}"
        prog_rule  = "LUCKY Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="lucky_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until LUCKY is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "lucky.key").exists() and (certs / "lucky.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in LUCKY to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('lucky_token','{tok}');
  sessionStorage.setItem('lucky_key','{key}');
  localStorage.setItem('lucky_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to LUCKY…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "lucky.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "lucky.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in LUCKY UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
